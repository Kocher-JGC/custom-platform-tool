...
class BasicLayout extends React.PureComponent<IBasicLayoutProps, IBaseLayoutState> {
  ...
  setDefaultTabs = (menu) => {
    const { query } = history.location;
    const { menuId, app, mode, lessee, pageId } = query;
    if (menuId) {
  ...
  IconAppointed = (props) => {
    ...
    if (typeof iconType === "string") {
      ...
      return RIcon ? <Icon component={() => <RIcon className={styles.menuIcon} />} /> : null;
    }
    return iconType;
  };

  loopMenuItemRender = (menus: MenuDataItem[]): React.ReactNode =>
    menus.map((menuItemProps) => {
      const { id, key, path, name, icon, children } = menuItemProps;
      const MenuIcon = icon ? this.IconAppointed({ iconType: icon }) : null;
      if (Array.isArray(children)) {
        return (
          <SubMenu key={key || id || path} icon={MenuIcon} title={name}>
      ...
      return (
        <Menu.Item
          key={key || id || path}
          icon={MenuIcon}
          onClick={() => this.handleMenuSelect(menuItemProps)}>
          {name}
        </Menu.Item>
      );
    });
  ...
  render() {
    const { settings, menuData, activeKey } = this.props;
...
export default connect(({ global, settings, menus, loading, tabs }: ConnectState) => {
  return {
