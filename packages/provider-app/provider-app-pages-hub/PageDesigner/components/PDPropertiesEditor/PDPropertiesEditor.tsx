...
const prepareData = async (widgetRef) => {
  const [widgetMeta, propItemGroupingData, propItemData] = await Promise.all([
...
export class PDPropertiesEditor extends React.Component<PropsEditorProps> {
  ...
  propItemRenderer = ({
    ...
    const { interDatasources, pageMetadata, selectedEntity } = this.props;
  ...
  render() {
    const { initEntityState, updateEntityState, entityState } = this.props;
