...
import { queryMenusListService, queryLesseeAuthorityListService } from '../service';
...
export const PrimaryTreeItem: React.FC<IProps> = React.memo((props: IProps): ReactElement => {
  ...
  * 表类型为 LesseeAuthority(普通表) TREE(树形表) 对应的数据
  */
  const getPrimaryTable = async (name = "") => {
    const res = await queryLesseeAuthorityListService({
      name,
      // type: `${TABLE_TYPE.LesseeAuthority},${TABLE_TYPE.TREE}`,
    });
