...
export const VariableEditor = ({
  ...
  const isDuplicated = (value, key) => {
    ...
    const amIDuplicated = Object.keys(variableList).some((type) => {
      const variables = variableList[type];
      const amIDuplicatedLoop = variables.some(
        (item) => (!data?.id || item.id !== data.id) && item[key] === value
      );
      return amIDuplicatedLoop;
    });
